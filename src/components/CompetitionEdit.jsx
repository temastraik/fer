...
import { CompetitionService } from '../services/competitionServiceEdit';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [disciplinesData, regionsData] = await Promise.all([
          CompetitionService.getDisciplines(),
          CompetitionService.getRegions()
        ]);
        ...
        const hasAccess = await CompetitionService.checkUserAccess(id, user?.id);
        if (!hasAccess) {
