...
import { fetchUserTeams, createNewTeam } from '../services/teamServicelist';
...
const TeamsList = () => {
  ...
  useEffect(() => {
    const loadTeams = async () => {
      ...
      try {
        setLoading(true);
        const { captainTeams, memberTeams } = await fetchUserTeams(user.id);
        
        setCaptainTeams(captainTeams);
        setMemberTeams(memberTeams);
        setTeams([...captainTeams, ...memberTeams]);
      } catch (error) {
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      setLoading(true);
      const newTeam = await createNewTeam(user.id, newTeamName.trim());
